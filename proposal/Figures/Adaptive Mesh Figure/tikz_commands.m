...
% adjusted mesh (moving all nodes) -- making a noisy mesh for me
ama = rp;
ama(:,3) = ama(:,3) + .1*randn(np,1);

% find the edges connected to the choosen node 
conN = f_FindN(index,t,ama);

% find normal and the end of the normal for each neighbor
% Na - actual unit normals
N = zeros(size(conN));
Na = zeros(size(conN));
Ne = zeros(size(conN));
for i = 1:size(conN,1)
    [~,n_index] = ismember(conN(i,:),ama,'rows'); 
    N(i,:) = f_Plane( f_FindN(n_index,t,ama) );
    if N(i,3)<0; N(i,:)=-N(i,:); end
    Na(i,:) = N(i,:) / norm(N(i,:));
    N(i,:)  = N(i,:) / (1.3*norm(N(i,:)));
    Ne(i,:) = roundn(conN(i,:) + N(i,:), -4); 
end
...
Bz = -.2*sin(By)+1.5;
BPc = f_MSplane(Bx,By,Bz,'blue!70!white');

% make curve for fitted plane
[Npc, Cpc] = f_Plane(rpc);
if Npc(3)>0; Npc=-Npc; end % want normal of point cloud plane to point down
Npc = Npc / norm(Npc);
Fz = Cpc(1)*Bx + Cpc(2)*By + Cpc(3);
NPpc = f_MSplane(Bx,By,Fz,'orange');

% adjust vertice - doesn't work. ugh!
Ncn = f_Plane(conN);
if Ncn(3)<0; Ncn=-Ncn; end
Ncn = Ncn / norm(Ncn);
Q = f_Q(Ncn,cn);
for i=1:size(conN,1); Q=Q+f_Q(Na(i,:),conN(i,:)); end
Q = Q+f_Q(Npc,mean(rpc));
A = Q(1:3,1:3);
b=Q(1:3,4);
w=-A^-1*b;
...
%% Figure showing samples and noisy mesh ________________________________ 1

fid = fopen('tikzFigs/AM1.tikz','w'); % open a file 
...
for i=1:np
    if i==index
        fprintf(fid,'\\fill[black] (%g,%g,%g) circle (1.8pt); \n',ama(i,1),ama(i,2),ama(i,3));
    else     
...
%% Figure normals and planes ____________________________________________ 2

fid = fopen('tikzFigs/AM2.tikz','w'); % open a file 
...
fprintf(fid,'\\draw[->, red, >=triangle 45, thick, line width = 2pt] (%g,%g,%g) -- (%g,%g,%g); \n'...
    ,conN(i,1),conN(i,2),conN(i,3),Ne(i,1),Ne(i,2),Ne(i,3));
end

% Dashed gray lines that show intersection 
for i=1:npc^2
    fprintf(fid,'\\fill[gray] (%g,%g,%g) circle (.4pt); \n',...
        rpc(i,1),rpc(i,2),Cpc(1)*rpc(i,1) + Cpc(2)*rpc(i,2) + Cpc(3));
end
fprintf(fid,'\\begin{scope}[opacity=.9, draw=gray, very thin, dashed]\n');
for i=1:npc^2
    fprintf(fid,'\\draw (%g,%g,%g) -- (%g,%g,%g); \n',...
        rpc(i,1),rpc(i,2),Cpc(1)*rpc(i,1) + Cpc(2)*rpc(i,2) + Cpc(3),rpc(i,1),rpc(i,2),rpc(i,3));
end
fprintf(fid,'\\end{scope}\n');
...
%% Figure normals and planes ____________________________________________ 3

fid = fopen('tikzFigs/AM3.tikz','w'); % open a file 
